...
let
  ...
  config = pkgs.writeTextDir "/helix/config.toml" (
    builtins.readFile (toml.generate "/config.toml" settings)
  );
  languages = pkgs.writeTextDir "/helix/langauges.toml" (
    builtins.readFile (import ./languages.nix { inherit pkgs; })
  );
